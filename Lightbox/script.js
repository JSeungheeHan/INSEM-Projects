...
function downloadHelp() {  
  window.scrollTo(0,0);
  var useWidth = document.getElementById("base").style.width;
  
  html2canvas(document.getElementById("base"), {width: useWidth, onrendered: function(canvas) {
...
function printHelper() {
  ...
  // popOut.addEventListener('afterprint', function() {
  //   console.log("Helaifhla");
  // });
   
  var doc = popOut.document;
  doc.open();
   //doc.write('<script type="text/javascript">addEventListener("load", () => { print(); popOut.onafterprint = function() { popOut.close(); } })</script></body></html>');
  doc.write("<link href=\"style.css\" rel=\"stylesheet\" type=\"text/css\" />");
  
   // popOut.document.write('<script type="text/javascript">addEventListener("load", () => { print(); close(); console.log(Success)};)</script></body></html>');
  //console.log("Failure");
  ...
  // popOut.onload = function() {popOut.print()};
  ...
  // doc.write(document.getElementById("total").outerHTML);
...
function addEstimate() {
  ...
  if (height < 8) {
    errorText = errorText + "Height must at least be 8\"" + "<br>";
  }
  if (width < 8) {
    errorText = errorText + "Width must at least be 8\"" + "<br>";
  }
  if(height > 360)
  {
    errorText = errorText + "Height can't exceed 30 feet";
  }
  if(width > 360)
  {
    errorText = errorText + "Width can't exceed 30 feet";
  }
  if (quantity <= 0 || quantity % 1 != 0 || height < 8 || width < 8 || height > 360 || width > 360) {
  ...
  item.array[saveStart].description += " : " + outputPer + " lm/ft";
